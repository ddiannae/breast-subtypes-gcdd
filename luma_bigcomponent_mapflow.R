...
load("networks/components-enrichment/luma_big_component.RData")
load("networks/components-enrichment/luma_communities_big_component.RData")
results.dir <- "networks/components-enrichment/"
...
mflow <- mapflow(big.component, net.comunities)
vtypes <- lapply(V(mflow)$name, function(n){
  sgrapg <- subgraph(big.component, net.comunities[[n]])
  type <- ifelse(mean(E(sgrapg)$Inter) > 0.5, "inter", "intra")
  prs <- page.rank(sgrapg)
  gene <- names(which.max(prs$vector))[1]
  return(list(type = type, gene = gene))
})

V(mflow)$type <- sapply(vtypes, "[[", 1)
V(mflow)$gen <-  sapply(vtypes, "[[", 2)

write.table(get.data.frame(mflow, what = "edges")[, c("from", "to", "weight")], 
            file = paste0(results.dir, "mapflow_luma_big_component_edges.tsv"), 
            row.names = F, quote = F, col.names = T, sep = "\t")
write.table(get.data.frame(mflow, what = "vertices"), 
            file = paste0(results.dir, "mapflow_luma_big_component_vertices.tsv"), 
            row.names = F, quote = F, col.names = T, sep = "\t")
